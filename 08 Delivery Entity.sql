...
create or replace table stage_sch.delivery (
    ...
    deliveredtime text,                     -- delivered time as text
...
copy into stage_sch.delivery (deliveryid,orderid, deliveryagentid, deliverystatus, 
                    estimatedtime, deliveredtime, addressid, deliverydate, createddate, 
...
from (
    select 
        ...
        t.$6::text as Deliveredtime,
        t.$7::text as addressid,
        t.$8::text as deliverydate,
    ...
    from @stage_sch.csv_stg/delivery.csv t
)
...
WHEN MATCHED THEN
    ...
    UPDATE SET
        ...
        delivered_time = source.deliveredtime,
