...
create or replace table stage_sch.customer (
    ...
    gender text ,                        -- gender as text
    dob text ,                           -- date of birth as text
...
copy into  stage_sch.customer (customerid, name, email, mobile, loginbyusing, gender, dob, anniversary,
                   Rating, preferences, createddate, modifieddate, 
...
from (
    select 
        ...
        t.$11::text as createddate,
        t.$12::text as modifieddate,
        metadata$filename as _stg_file_name,
        metadata$file_last_modified as _stg_file_load_ts,
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_sch.csv_stg/customer.csv t
)
file_format = (format_name = 'stage_sch.csv_file_format')
on_error = abort_statement;

select * from SANDBOX.STAGE_SCH.CUSTOMER;
...
CREATE OR REPLACE TABLE CLEAN_SCH.CUSTOMER (
    ...
    EMAIL STRING(100),                                           -- Mobile number, accommodating international format
    MOBILE STRING(15),                                           -- Email
    LOGIN_BY_USING STRING(50),                                   -- Method of login (e.g., Social, Google, etc.)
    GENDER STRING(10),                                           -- Gender
    DOB DATE,                                                    -- Date of birth in DATE format
    ANNIVERSARY DATE,                                            -- Anniversary in DATE format
    RATING FLOAT,
    PREFERENCES variant,                                          -- Customer preferences
...
select 
    ...
    rating::float,
    preferences::VARIANT,
    createddate::TIMESTAMP_NTZ as created_dt,  -- timestamp conversion
    modifieddate::TIMESTAMP_NTZ as modified_dt, -- timestamp conversion
    _stg_file_name,
    _stg_file_load_ts,
    _stg_file_md5,
    _copy_data_ts
