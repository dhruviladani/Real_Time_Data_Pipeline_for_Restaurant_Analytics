...
create or replace table sandbox.stage_sch.restaurant (
    restaurantid text,      
    ...
    Ratings float,  --Ratings of restaurant
    Coupons variant, -- Coupons for discount for each restaurant
...
from (
    select 
     ...
     from @sandbox.stage_sch.csv_stg/restaurant.csv
 t
)
...
create or replace table sandbox.clean_sch.restaurant (
...
create or replace stream sandbox.clean_sch.restaurant_stm 
...
insert into sandbox.clean_sch.restaurant (
    restaurant_id,
    name,
    cuisine_type,
    pricing_for_two,
...
MERGE INTO sandbox.clean_sch.restaurant AS target
USING (
    SELECT 
        try_cast(restaurantid AS number) AS restaurant_id,
        try_cast(name AS string) AS name,
...
-- -- ********************* aa vali file nai etle run nai thai 
...
--         metadata$file_content_key as _stg_file_md5,
--         current_timestamp() as _copy_data_ts
--      from @SWIGGY_DATA.PUBLIC.SWIGGY_DATA_EXTERNAL_STAGE/daily/restaurant/day-02-upsert-restaurant-delhi+NCR.csv t
-- )
-- file_format = (format_name = 'stage_sch.csv_file_format')
-- on_error = abort_statement;
